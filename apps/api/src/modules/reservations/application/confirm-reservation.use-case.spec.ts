...
function makeReservation(statusOverride?: ReservationStatus): Reservation {
  ...
  if (statusOverride !== undefined) r.status = statusOverride;
  return r;
}
...
describe('ConfirmReservationUseCase', () => {
  it('confirms a pending reservation', async () => {
    ...
    const useCase = new ConfirmReservationUseCase(repo as never);
  ...
  it('throws BadRequestException when reservation is cancelled', async () => {
    const reservation = makeReservation();
